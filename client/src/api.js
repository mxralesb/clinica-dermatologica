...
// Compat opcional si aún usas apiLogin en algún lado
export function apiLogin(emailOrObj, password, type) {
  let email, pass, kind
  if (typeof emailOrObj === 'object') { email=emailOrObj.email; pass=emailOrObj.password; kind=(emailOrObj.type||emailOrObj.role||'DERM').toUpperCase() }
  else { email=emailOrObj; pass=password; kind=(type||'DERM').toUpperCase() }
